...
// used to toggle between modes for huntKey
typedef enum {
  find_replace,  // find and replace
  find,  // find, do not replace/remove
  find_remove  // find and remove
} Mode;

// helper function to find and (optionally) replace/remove a given key in a
// given chain. Returns true if something was found and replaced, false if not
// found. Trusts caller (me) to not pass in a dummy newkeyvalue with a NULL
// value if I want to find and replace.
// args:
// chain - chain we're working with
// newkeyvalue - pair whose key we will compare against, and value to replace
// oldkeyvalue - output param to return any keyvalue
// doReplace - find_replace: find and replace, find: find only and don't
//             replace, remove: find and remove
static bool huntKey(LinkedList *chain,
  ...
  HTKey_t key = newkeyvalue.key;
  LLIterator *iter = LLIterator_Allocate(chain);

  while (LLIterator_IsValid(iter)) {
    HTKeyValue_t *replace;
    LLIterator_Get(iter, (LLPayload_t*) &replace);
    if (replace->key == key) {
      *found = *replace;
      if (mode == find_replace) {
        replace->value = newkeyvalue.value;
      } else if (mode == find_remove) {
...
bool HashTable_Insert(HashTable *table,
  ...
  // if key found replace and return true
  if (huntKey(chain, newkeyvalue, oldkeyvalue, find_replace)) {
    return true;
  }
  // if key not found, insert as normal
  HTKeyValue_t *insert = malloc(sizeof(HTKeyValue_t));
  Verify333(insert != NULL);
  *insert = newkeyvalue;
  LinkedList_Append(chain, (LLPayload_t) insert);
...
bool HashTable_Find(HashTable *table,
  ...
  // creating a dummy key value pair without a value to satisfy huntKey arg
  // types. Since we are only finding, we don't need the value, only the key.
  HTKeyValue_t dummy;
  dummy.key = key;
  dummy.value = NULL;
  ...
  return huntKey(chain, dummy, keyvalue, find);
}
...
bool HashTable_Remove(HashTable *table,
  ...
  if (huntKey(chain, dummy, keyvalue, find_remove)) {
    table->num_elements -= 1;
    return true;
  }
...
bool HTIterator_Next(HTIterator *iter) {
  ...
  // case 1: can iterate within the bucket
  if (LLIterator_Next(iter->bucket_it)) {
    return true;
  }

  // case 2: cannot iterate within this bucket, move iter to next bucket
  LLIterator_Free(iter->bucket_it);
  iter->bucket_it = NULL;
  ...
  // case 3: no other buckets to iterate through
  iter->bucket_it = NULL;
  iter->bucket_idx = INVALID_IDX;
  return false;
}
