...
class Test_DocTableReader : public ::testing::Test {
 protected:
  ...
  list<IndexFileOffset_t> LookupElementPositions(DocID_t hash_val) {
...
TEST_F(Test_DocTableReader, TestDocTableReaderBasic) {
  ...
  ASSERT_TRUE(success);
  ASSERT_EQ(string("test_tree/enron_email/102"), str);
  success = dtr_->LookupDocID(55, &str);
  ASSERT_TRUE(success);
  ASSERT_EQ(string("test_tree/enron_email/149"), str);
