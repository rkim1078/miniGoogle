...
TEST(Test_DocIDTableReader, TestDocIDTableReaderBasic) {
  ...
  DocIDTableReader* ditr = itr.LookupWord(string("happy"));
  ...
  map<DocID_t, int32_t> terminal_results {
    {447U, 6},
    {733U, 1},
    {801U, 1}
  };
  map<DocID_t, int32_t> sun_results {
    {631U, 3},
    {635U, 2},
    {575U, 2},
    {630U, 1}
  };
  map<DocID_t, int32_t> careful_results {
    {203U, 1},
    {785U, 1}
  };
  ...
  ditr = itr.LookupWord(string("terminal"));
  ASSERT_NE(static_cast<DocIDTableReader*>(nullptr), ditr);
  list<DocIDElementHeader> doc_id_list = ditr->GetDocIDList();
  ...
  ditr = itr.LookupWord(string("sun"));
  ASSERT_NE(static_cast<DocIDTableReader*>(nullptr), ditr);
  ...
  ditr = itr.LookupWord(string("careful"));
  ASSERT_NE(static_cast<DocIDTableReader*>(nullptr), ditr);
  doc_id_list = ditr->GetDocIDList();
  ...
  ditr = itr.LookupWord(string("echoes"));
