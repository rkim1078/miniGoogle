...
TEST(Test_QueryProcessor, TestQueryProcessorSingleIndex) {
  ...
  ASSERT_EQ(string("test_tree/books/ulysses.txt"),
            res[1].document_name);
  ASSERT_EQ(2, res[1].rank);
  ASSERT_EQ(string("test_tree/books/warandpeace.txt"),
            res[2].document_name);
...
TEST(Test_QueryProcessor, TestQueryMultiIndex) {
  ...
  ASSERT_EQ(2U, res.size());
  ASSERT_EQ(string("test_tree/books/artofwar.txt"),
            res[0].document_name);
  ASSERT_EQ(8, res[0].rank);
  ASSERT_EQ(string("test_tree/enron_email/613"),
            res[1].document_name);
  ASSERT_EQ(2, res[1].rank);
  ...
  res = qp.ProcessQuery(query);
  ASSERT_EQ(3U, res.size());
  ASSERT_EQ(string("test_tree/books/mobydick.txt"),
            res[0].document_name);
