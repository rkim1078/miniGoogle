...
TEST(Test_HttpUtils, TestHttpUtilsIsPathSafe) {
  ...
  ASSERT_FALSE(IsPathSafe(basedir, file7_bad));
...
TEST(Test_HttpUtils, TestHttpUtilsWrappedReadWrite) {
  const char *kFileAsString = "This is a test; this is only a test.\n";
  const unsigned char *kFiledata
    = reinterpret_cast<const unsigned char *>(kFileAsString);
  const int kFiledataLen = strlen(kFileAsString);
  ...
  int file_fd = open("test_files/test.txt",
                     ...
                     S_IRUSR | S_IWUSR);
  ASSERT_NE(-1, file_fd);
  ASSERT_EQ(kFiledataLen,
            WrappedWrite(file_fd, kFiledata, kFiledataLen));
  close(file_fd);
  ...
  ASSERT_NE(-1, file_fd);
  ASSERT_EQ(kFiledataLen,
            WrappedRead(file_fd, readdata, kFiledataLen));
  close(file_fd);
  ASSERT_EQ(0, memcmp(kFiledata, readdata, kFiledataLen));
