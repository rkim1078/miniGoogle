...
class ServerSocket {
 public:
  ...
  // This function causes the ServerSocket to attempt to accept an incoming
  // connection from a client.  BindAndListen() must have been successfully
  // called prior to invoking this, as the client arrives via the
  // listening socket which was set up in that method.  If there is no
  // currently waiting client, this method WILL BLOCK until a client
  // arrives.  On failure, returns false.  On success, returns true and also
  // returns (via output parameters) the following:
  //
