...
static void GetPortAndPath(int argc,
  ...
  // parse port number (argv[1])
  int p = atoi(argv[1]);
  // ensure port is in a reasonable range (1024–65535)
  if (p < 1024 || p > 65535) {
    Usage(argv[0]);
  }
  *port = static_cast<uint16_t>(p);

  // make sure static files dir is readable (argv[2])
  DIR *d = opendir(argv[2]);
  if (d == nullptr) {
    Usage(argv[0]);  // not readable
  }
  closedir(d);
  *path = string(argv[2]);

  // make sure indices are readable (argv[3+])
