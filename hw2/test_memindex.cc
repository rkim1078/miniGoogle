...
TEST(Test_MemIndex, Integration) {
  ...
  ASSERT_EQ(2, LinkedList_NumElements(ll_res));
  ...
  for (i = 0; i < LinkedList_NumElements(ll_res); i++) {
    ...
    if (i == 0) {
      ...
      ASSERT_EQ(0,
                strcmp(doc_name, "./test_tree/bash-4.2/support/texi2html"));
    } else if (i == 1) {
      char* doc_name = DocTable_GetDocName(dt, res->doc_id);
      ASSERT_EQ(0,
                strcmp(doc_name, "./test_tree/bash-4.2/support/man2html.c"));
    }
    LLIterator_Next(lit);
  }
  LLIterator_Free(lit);
  ...
  ASSERT_NE((LinkedList*) NULL, ll_res);
  ASSERT_EQ(1, LinkedList_NumElements(ll_res));
  lit = LLIterator_Allocate(ll_res);
  for (i = 0; i < LinkedList_NumElements(ll_res); i++) {
    SearchResult* res;
...
TEST(Test_MemIndex, Simple) {
  ...
  ASSERT_EQ((LinkedList*) NULL, (LinkedList*)results);
  ...
  ASSERT_NE((LinkedList*) NULL, results);
